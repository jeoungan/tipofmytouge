...
assert.match(css, /\.mode-grid[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
assert.match(css, /\.mode-card\.easy/);
assert.match(css, /\.mode-card\.challenge/);
assert.match(css, /\.mode-icon[\s\S]*width: 60px/);
assert.match(css, /\.mode-icon-label/);
assert.match(css, /\.mode-grid[\s\S]*transform: translateY\(-58px\)/);
