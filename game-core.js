(function initGameCore(global) {
  ...
  const words = [
    ...
    {
      answer: "만델브로 집합",
      aliases: ["만델브로트 집합", "Mandelbrot set"],
      mode: "challenge",
      clues: [
        "아, 그거 있잖아. 수학 그림인데, 확대하면 계속 비슷한 괴상한 무늬가 또 나오는 그거.",
        "복소수 쪽에서 나오는 건데, 점 하나하나가 어떤 계산을 반복해도 도망 안 가는지 보는 느낌이야.",
        "검은 덩어리 주변에 알록달록한 프랙털 그림으로 많이 보여. 컴퓨터 배경화면 같은 데서 봤을 수도 있어.",
        "이름이 어떤 수학자 성에서 왔고, z를 제곱하고 c를 더하는 반복식이 핵심인 그 집합.",
        "프랙털 대표 선수 같은 거. 경계가 미친 듯이 복잡해서 확대할수록 새 모양이 계속 튀어나와."
      ]
    },
    {
      answer: "크리스퍼",
      aliases: ["CRISPR", "CRISPR-Cas9", "유전자 가위"],
      mode: "challenge",
      clues: [
        "아, 그거 있잖아. 생물학 쪽에서 DNA를 원하는 데서 싹 자르는 도구처럼 말하는 그거.",
        "원래는 세균이 바이러스 기억해뒀다가 막는 면역 시스템 같은 데서 나온 아이디어야.",
        "가이드 RNA가 길잡이처럼 붙고, Cas9 같은 단백질이 실제로 자르는 역할을 하는 쪽.",
        "유전자 편집 이야기 나오면 거의 바로 튀어나오는 이름이고, 질병 치료나 작물 연구에서 자주 언급돼.",
        "별명이 유전자 가위인 그 기술. 정확히는 반복 서열 이름에서 온 영어 약자야."
      ]
    }
  ];
